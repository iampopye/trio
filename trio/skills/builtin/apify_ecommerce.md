...
- `.env` file with `APIFY_TOKEN` (at `~/.trio/.env`)
...
```bash
SKILL_PATH=~/.trio/skills/apify-ecommerce
```
...
```bash
node --env-file=~/.trio/.env $SKILL_PATH/reference/scripts/run_actor.js \
...
| Error | Solution |
|-------|----------|
| `APIFY_TOKEN not found` | Ensure `~/.trio/.env` contains `APIFY_TOKEN=your_token` |
