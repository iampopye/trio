...
Review bot feedback (from Sentry, Warden, Trio, Bugbot, CodeQL, etc.) appears in `high`/`medium`/`low` with `review_bot: true` — it is NOT placed in the `bot` bucket.
...
- Before replying, check if the thread already has a reply ending with `*- Trio*` or `*— Trio*` to avoid duplicates on re-loops
