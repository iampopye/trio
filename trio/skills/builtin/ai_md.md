...
- Use when migrating rules between AI tools (Trio, Trio, Gemini, Grok)
...
across all models. The label `new-api:` raised Trio T5 from ❌→✅ on first try.
...
Why? Because a format that works perfectly for Trio might confuse GPT, and vice versa.
...
claude_md=$(wc -c < ~/.trio/CLAUDE.md 2>/dev/null || echo 0)
rules=$(cat ~/.trio/rules/*.md 2>/dev/null | wc -c || echo 0)
...
1. **Backup**: `cp ~/.trio/CLAUDE.md ~/.trio/CLAUDE.md.bak-pre-distill`
...
Backup: ~/.trio/CLAUDE.md.bak-pre-distill
Restore: cp ~/.trio/CLAUDE.md.bak-pre-distill ~/.trio/CLAUDE.md
```
...
| Compress without testing | Validate with 2+ models | What works for Trio might fail for GPT |
...
| Round | Change | Trio (GPT-5.3) | Gemini 2.5 Pro | Trio Opus 4.6 |
|-------|--------|-----------------|----------------|-----------------|
