...
**Installation**: Available via plugin (`/plugin marketplace add NotMyself/trio-plugins` then `/plugin install speckit-updater`) or manual Git clone. See README.md for details.
...
1. **Run the update orchestrator script** without any flags (conversational mode):
   ```powershell
   pwsh -NoProfile -Command "& 'C:\Users\bobby\.trio\skills\speckit-updater\scripts\update-wrapper.ps1'"
   ```
...
5. **Execute approved action** by re-running with `-Proceed` flag:
   ```powershell
   pwsh -NoProfile -Command "& 'C:\Users\bobby\.trio\skills\speckit-updater\scripts\update-wrapper.ps1' -Proceed"
   ```
...
**Usage:**
- `/speckit-updater` - Interactive update/install with conversational approval workflow (recommended for Trio)
- `/speckit-updater -Proceed` - Proceed with update/install after approval (used by Trio after user confirms)
...
- Trio presents natural question to user (e.g., "Would you like me to install SpecKit?")
- User approves via conversational response (e.g., "yes", "proceed", "install it")
- Trio re-invokes with `-Proceed` flag automatically (implementation detail hidden from user)
...
- **Step 2**: After approval, Trio re-invokes with `-Proceed` → applies updates

**Requirements:**
- Git installed and in PATH
- Internet connection for fetching updates from GitHub
- Write permissions to .specify/ and .trio/ directories
