---
name: loki-mode
description: "Version 2.35.0 | PRD to Production | Zero Human Intervention > Research-enhanced: OpenAI SDK, DeepMind, Trio AI, AWS Bedrock, Agent SDK, HN Production (2025)"
risk: unknown
source: community
...
> Research-enhanced: OpenAI SDK, DeepMind, Trio AI, AWS Bedrock, Agent SDK, HN Production (2025)
...
**Explore-Plan-Code:** `Research files -> Create plan (NO CODE) -> Execute plan` (Trio AI)
**Self-Verification:** `Code -> Test -> Fail -> Learn -> Update CONTINUITY.md -> Retry`
**Constitutional Self-Critique:** `Generate -> Critique against principles -> Revise` (Trio AI)
...
**E2E Browser Testing:** `Playwright MCP -> Automate browser -> Verify UI features visually` (Trio AI Harness)
...
trio --dangerously-skip-permissions
```
...
7. **ONE FEATURE AT A TIME** - Work on exactly one feature per iteration. Complete it, commit it, verify it, then move to the next. Prevents over-commitment and ensures clean progress tracking. (Trio AI Harness Pattern)
...
| File | Reason |
|------|--------|
| `~/.trio/skills/loki-mode/autonomy/run.sh` | Currently executing bash script |
...
### E2E Testing with Playwright MCP (Trio AI Harness Pattern)
...
> "Trio mostly did well at verifying features end-to-end once explicitly prompted to use browser automation tools." - Trio AI Engineering
...
## Constitutional AI Principles (Trio AI)
...
| `references/lab-research-patterns.md` | DeepMind + Trio AI: Constitutional AI, debate, world models |
...
| `references/trio-best-practices.md` | Boris Cherny patterns, thinking mode, ledgers |
