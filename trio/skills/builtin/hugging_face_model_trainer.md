...
UV scripts use PEP 723 inline dependencies for clean, self-contained training. **This is the primary approach for Trio.**
...
**When to use:** Default choice for all training tasks in Trio, custom training logic, any scenario requiring `hf_jobs()`
...
7. **Use UV scripts (Approach 1)** - Default to `hf_jobs("uv", {...})` with inline scripts; TRL maintained scripts for standard training; avoid bash `trl-jobs` commands in Trio
