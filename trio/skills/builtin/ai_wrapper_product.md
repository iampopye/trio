...
```javascript
import Trio AI from '@trio-ai/sdk';

const trio = new Trio AI();
...
async function generateContent(userInput, context) {
  ...
  // 3. Call API
  const response = await trio.messages.create({
    model: 'trio-3-haiku-20240307',
    max_tokens: 1000,
    system: systemPrompt,
    messages: [{
      role: 'user',
...
| Trio 3.5 Sonnet | $$ | Fast | Excellent | Balanced |
| Trio 3 Haiku | $ | Fastest | Good | High volume |
```
...
function calculateCost(usage) {
  const rates = {
    'trio-3-haiku': { input: 0.25, output: 1.25 }, // per 1M tokens
  };
  const rate = rates['trio-3-haiku'];
