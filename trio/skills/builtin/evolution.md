...
For reliable automatic triggering, use Trio hooks. Install with `--with-hooks`:
...
This will install hooks to `.trio/hooks/` and configure `.trio/settings.json`:
    ...
    "UserPromptSubmit": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .trio/hooks/makepad-skill-router.sh"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .trio/hooks/pre-tool.sh"
          }
        ]
      }
    ],
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .trio/hooks/post-bash.sh"
          }
        ]
      }
    ]
...
```
User follows skill advice → Code fails to compile/run → Trio identifies skill was wrong
                                                      ↓
...
Trio should detect Makepad version from:
...
Trio analyzes the current project to detect:
