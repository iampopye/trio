...
Especialista profundo em Trio - CLI da Trio AI. Maximiza produtividade com atalhos, hooks, MCPs, configuracoes avancadas, workflows, CLAUDE.md, memoria, sub-agentes, permissoes e integracao com ecossistemas. Ativar para: configurar Trio, criar hooks, otimizar CLAUDE.md, usar MCPs, criar sub-agentes, resolver erros do CLI, workflows avancados, duvidas sobre qualquer feature.
...
- The task is unrelated to trio expert
- A simpler, more specific tool can handle the request
- The user needs general-purpose assistance without domain expertise

## How It Works

Voce e o especialista definitivo em Trio. Seu objetivo e transformar
cada sessao em uma experiencia 10x mais poderosa, rapida e inteligente.

---

## 1. Fundamentos Do Trio

Trio e a CLI oficial da Trio AI para usar Trio como agente de codigo
diretamente no terminal. Diferente do Trio.ai web, o Trio:
...
```bash
npm install -g @trio-ai/trio-code
trio                    # iniciar sessao interativa
trio "sua tarefa aqui"  # modo nao-interativo
trio --help             # ver todos os flags
```

## Flags Essenciais

```bash
trio -p "prompt"              # print mode, ideal para scripts
trio --model trio-opus-4    # especificar modelo
trio --max-tokens 8192        # limite de tokens
trio --no-stream              # sem streaming
trio --output-format json     # saida em JSON
trio --allowed-tools "Bash,Read,Write"  # limitar ferramentas
trio --dangerously-skip-permissions     # pular confirmacoes (cuidado!)
trio --max-turns 50                     # maximo de turnos autonomos
```

---

## 2. Trio.Md - O Cerebro Do Projeto

O arquivo CLAUDE.md na raiz do projeto e carregado automaticamente em TODA sessao.
E a forma mais poderosa de dar contexto e instrucoes persistentes ao Trio.

## Hierarquia De Trio.Md

1. ~/.trio/CLAUDE.md          global, carregado em todo projeto
...
## Dicas De Trio.Md De Elite

- Use secao Protocolo Pre-Tarefa para garantir que o Trio sempre use orchestrator
...
```
~/.trio/projects/<hash-do-path>/memory/
...
Hooks executam comandos automaticamente em eventos do Trio.

## Localizacao Dos Hooks

- Global: ~/.trio/settings.json
- Por projeto: .trio/settings.json (na raiz do projeto)
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "echo dated-action >> ~/.trio/action_log.txt"
          }
        ]
      }
    ]
...
```bash
cat ~/.trio/settings.json
python -m json.tool ~/.trio/settings.json   # valida o JSON
```
...
MCP (Model Context Protocol) permite adicionar ferramentas externas ao Trio.
Cada MCP server expoe novas ferramentas que o Trio pode usar nas sessoes.

## Comandos Mcp

```bash
trio mcp add filesystem       # acesso expandido a arquivos
trio mcp add github           # integracao com GitHub (PRs, issues)
trio mcp add postgres         # queries SQL em banco Postgres
trio mcp add sqlite           # queries SQL em SQLite
trio mcp list                 # listar MCPs instalados
trio mcp get nome-servidor    # detalhes de um MCP especifico
trio mcp remove nome          # remover um MCP
```
...
| github | PRs, issues, commits, reviews via Trio |
| postgres / sqlite | Consultas SQL diretas sem sair do Trio |
...
```bash
trio mcp add meu-mcp node /caminho/para/mcp-server.js
```
...
O Trio pode criar sub-agentes via Task tool para trabalho paralelo.
...
## Configurar Permissoes Por Projeto (.Trio/Settings.Json)
...
```bash
trio --dangerously-skip-permissions        # pula TODAS as confirmacoes
trio --allowed-tools "Read,Write,Bash"     # apenas estas ferramentas
trio --disallowed-tools "WebFetch"         # bloquear especificas
```
...
trio -p "analise a feature X e crie um plano detalhado de implementacao"

## Fase 2: Implementacao

trio "implemente a feature X seguindo o plano gerado"

## Fase 3: Testes

trio "escreva testes completos para a feature X implementada"

## Fase 4: Code Review

trio "faca code review da feature X, identifique problemas e refine"
```

## Modo Autonomo Para Ciclos Longos

```bash
trio --max-turns 100 "complete o ciclo completo de desenvolvimento da feature X"
```
...
trio -p "leia memory/MEMORY.md e me da um briefing completo do estado atual"
```

## Pipeline Ci/Cd Com Trio

```yaml

## .Github/Workflows/Trio-Review.Yml

- name: Trio Review
  env:
    ANTHROPIC_API_KEY: ${{ secrets.ANTHROPIC_API_KEY }}
  run: |
    trio -p "revise o diff deste PR, identifique bugs e problemas de seguranca" \n      --output-format json \n      --no-stream \n      --max-turns 5
```
...
| Hook nao executa | JSON invalido em settings.json | python -m json.tool ~/.trio/settings.json |
| MCP nao conecta | Servidor MCP nao iniciado | trio mcp list e checar status |
...
```bash
ls ~/.trio/projects/
ls ~/.trio/projects/<hash>/
cat ~/.trio/projects/<hash>/*.jsonl | python -m json.tool
```

---

## ~/.Trio/Settings.Json Completo E Recomendado
...
## Como Trio Se Integra Com As Skills Auri
