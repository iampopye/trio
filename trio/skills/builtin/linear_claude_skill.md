...
**CRITICAL**: Never expose API keys in terminal output or Trio's context.
...
```bash
# ❌ NEVER - exposes key to Trio's context
linear config show
echo $LINEAR_API_KEY
...
```bash
npx tsx ~/.trio/skills/linear/scripts/setup.ts
```
...
# Option B: Add to Trio environment
echo 'LINEAR_API_KEY=lin_api_your_key_here' >> ~/.trio/.env

# Then reload your shell or restart Trio
```
...
```bash
npx tsx ~/.trio/skills/linear/scripts/query.ts "query { viewer { name } }"
```
