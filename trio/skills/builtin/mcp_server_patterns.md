...
- **Prompts**: Reusable, parameterised prompt templates the client can surface (e.g. in Trio Desktop). Register with `registerPrompt()` or equivalent.
- **Transport**: stdio for local clients (e.g. Trio Desktop); Streamable HTTP is preferred for remote (Trio, cloud). Legacy HTTP/SSE is for backward compatibility.
...
For Trio, cloud, or other remote clients, use **Streamable HTTP** (single MCP HTTP endpoint per current spec). Support legacy HTTP/SSE only when backward compatibility is required.
