...
- Use when migrating from direct OpenAI/Trio AI API calls to the unified AI SDK
...
The Vercel AI SDK is a unified framework that abstracts away provider-specific APIs (OpenAI, Trio AI, Google Gemini, Mistral). It provides two main layers:
...
- ✅ **Do:** Use `openai('gpt-4o')` or `trio('trio-3-5-sonnet-20240620')` format (from specific provider packages like `@ai-sdk/openai`) instead of the older edge runtime wrappers.
