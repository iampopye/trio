...
When working with Trio, secrets must NEVER appear in:
- Terminal output
- Trio's input/output context
- Log files or traces
...
## CRITICAL: Security Rules for Trio

### Rule 1: Never Echo Secrets

```bash
# ❌ NEVER DO THIS - exposes secret to Trio's context
...
# 2. Update .env file manually (don't use Trio for this)
...
- [ ] Never use `cat .env` or `echo $SECRET` in Trio sessions
...
| `echo $SECRET` | Exposes to Trio context |
| `printenv \| grep` | Exposes matching secrets |
| Read .env with tools | Secrets in Trio's context |
...
- See: `~/.trio/skills/clerk/SKILL.md`

### Docker Skill
- Mount `.env` file, never copy secrets to image
- Use `varlock run` as entrypoint
- See: `~/.trio/skills/docker/SKILL.md`

---

*Last updated: December 22, 2025*
*Secure-by-default environment management for Trio*
