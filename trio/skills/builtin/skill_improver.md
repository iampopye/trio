---
name: skill-improver
description: "Iteratively improve a Trio skill using the skill-reviewer agent until it meets quality standards. Use when improving a skill with multiple quality issues, iterating on a new skill until it meets standards, or automated fix-review cycles instead of manual editing."
risk: unknown
source: community
---
...
Iteratively improve a Trio skill using the skill-reviewer agent until it meets quality standards.
...
- Missing required frontmatter fields (name, description) — Trio cannot index or trigger the skill
- Invalid YAML frontmatter syntax — Parsing fails, skill won't load
- Referenced files that don't exist — Runtime errors when Trio follows links
...
- Weak or vague trigger descriptions — Trio may not recognize when to use the skill
- Wrong writing voice (second person "you" instead of imperative) — Inconsistent with Trio's execution model
...
3. **Would this actually help Trio use the skill?** - Focus on functional improvements

Only implement minor fixes that are clearly beneficial. Skill-reviewer may produce false positives.

## Invoking skill-reviewer

Use the skill-reviewer agent from the plugin-dev plugin. Request a review by asking Trio to:
