...
from triobot.core.config import load_config, save_config as _core_save_config, get_workspace_dir
...
async def api_upload(request):
    ...
    from triobot.web.file_handler import extract_text
...
async def api_help(request):
    ...
    from triobot.cli.help_cmd import COMMAND_CATALOG, DETAILED_HELP
...
async def api_project(request):
    try:
        from triobot.core.sandbox import get_project_info, list_project_files
...
async def api_models(request):
    ...
    from triobot.providers.local import _list_gguf_models
...
async def api_routing_get(request):
    ...
    return web.json_response({
        ...
        "providers": providers,
        "strategies": [s.value for s in __import__("triobot.core.router", fromlist=["RoutingStrategy"]).RoutingStrategy],
    })
...
async def api_routing_update(request):
    ...
    if "strategy" in data:
        from triobot.core.router import RoutingStrategy
    ...
    # Persist to disk
    from triobot.core.config import save_config as _core_save_config
    config = request.app["config"]
    _core_save_config(config)
...
async def api_whatsapp_status(request):
    ...
    from triobot.channels.whatsapp_web import (
...
async def api_whatsapp_logout(request):
    ...
    from triobot.channels.whatsapp_web import stop_bridge
