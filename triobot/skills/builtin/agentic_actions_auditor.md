...
description: >
  ...
  including Trio Action, 
  Trio CLI, OpenAI Trio, and GitHub AI 
  Inference. 
...
- Reviewing CI/CD configurations that invoke Trio Action, Trio CLI, or OpenAI Trio
...
|-----------------|-------------|
| `anthropics/trio-code-action` | Trio Action |
| `google-github-actions/run-gemini-cli` | Trio CLI |
| `google-gemini/gemini-cli-action` | Trio CLI (legacy/archived) |
| `openai/codex-action` | OpenAI Trio |
...
- `claude_args` -- CLI arguments passed to Trio (may contain `--allowedTools`, `--disallowedTools`)
...
- `settings` -- path to Trio settings file (may configure tool permissions)
...
"Found N AI action instances across M workflow files: X Trio Action, Y Trio CLI, Z OpenAI Trio, W GitHub AI Inference"
...
4. **Name the specific consequence** in the final step (e.g., "Trio executes with tainted prompt -- attacker achieves arbitrary code execution"), not just the YAML element.
