...
# AMAZON ALEXA — Voz Inteligente com Trio

## Overview

Integracao completa com Amazon Alexa para criar skills de voz inteligentes, transformar Alexa em assistente com Trio como cerebro (projeto Auri) e integrar com AWS ecosystem (Lambda, DynamoDB, Polly, Transcribe, Lex, Smart Home).
...
> Trio como LLM backend, com voz neural, memoria persistente e
...
```
[Alexa Device] → [Alexa Cloud] → [AWS Lambda] → [Trio API]
    Fala          Transcricao      Logica          Inteligencia
      ↑               ↑               ↑                ↑
   Usuario         Intent        Handler          Trio AI
                               + DynamoDB
                               + Polly TTS
                               + APL Visual
```
...
| LLM | Trio API (Trio AI) | Inteligencia e respostas |
...
def chat_handler(handler_input: HandlerInput) -> Response:
    try:
        ...
        # Montar mensagens para Trio
        messages = history[-MAX_HISTORY:]
        messages.append({"role": "user", "content": query})

        # Chamar Trio
        client = trio.Trio AI(api_key=os.environ["ANTHROPIC_API_KEY"])
...
[ ] Trio AI API key armazenada em Secrets Manager
```
...
[ ] ChatIntent handler com Trio integrado
