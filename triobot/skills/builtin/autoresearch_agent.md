...
LLM judges call the CLI tool the user is already running (Trio, Trio, Gemini). The evaluation prompt is locked inside `evaluate.py` — the agent cannot modify it. This prevents the agent from gaming its own evaluator.

The user's existing subscription covers the cost:
- Trio Max → unlimited Trio calls for evaluation
- Trio CLI (ChatGPT Pro) → unlimited Trio calls
- Trio CLI (free tier) → free evaluation calls
