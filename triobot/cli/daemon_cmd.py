...
from triobot.core.config import get_trio_dir
...
def _get_trio_command() -> str:
    ...
    if trio_bin:
        return trio_bin
    return f"{sys.executable} -m triobot"
...
def _cmd_start():
    ...
    from triobot.cron.daemon import TrioDaemon
    ...
    cmd = [_pythonw_exe(), "-m", "triobot.cron.daemon"]
...
<dict>
    ...
    <array>
        ...
        <string>-m</string>
        <string>triobot.cron.daemon</string>
    </array>
    <key>RunAtLoad</key>
    <true/>
    <key>KeepAlive</key>
...
[Service]
Type=simple
ExecStart={python} -m triobot.cron.daemon
...
def _install_windows():
    ...
    vbs_content = (
        ...
        f'WshShell.Run """{pythonw}"" -m triobot.cron.daemon", 0, False\r\n'
    )
    ...
    result = subprocess.run(  # nosec B603 B607
        [
            "schtasks", "/Create",
            "/TN", _WIN_TASK_NAME,
            "/TR", f'"{pythonw}" -m triobot.cron.daemon',
            "/SC", "ONLOGON",
            "/RL", "HIGHEST",
            "/F",
        ],
