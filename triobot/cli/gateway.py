...
from triobot.core.config import load_config, get_agent_defaults
...
from triobot.channels.base import ChannelManager
from triobot.providers.base import register_all_providers, ProviderRegistry
...
async def run_gateway():
    ...
    if channels_config.get("discord", {}).get("enabled"):
        try:
            from triobot.channels.discord_channel import DiscordChannel
    ...
    if channels_config.get("telegram", {}).get("enabled"):
        try:
            from triobot.channels.telegram_channel import TelegramChannel
    ...
    if channels_config.get("signal", {}).get("enabled"):
        try:
            from triobot.channels.signal_channel import SignalChannel
    ...
    if channels_config.get("whatsapp", {}).get("enabled"):
        try:
            from triobot.channels.whatsapp_channel import WhatsAppChannel
    ...
    if channels_config.get("slack", {}).get("enabled"):
        try:
            from triobot.channels.slack_channel import SlackChannel
    ...
    if channels_config.get("teams", {}).get("enabled"):
        try:
            from triobot.channels.teams_channel import TeamsChannel
    ...
    if channels_config.get("google_chat", {}).get("enabled"):
        try:
            from triobot.channels.google_chat_channel import GoogleChatChannel
    ...
    if channels_config.get("imessage", {}).get("enabled"):
        try:
            from triobot.channels.imessage_channel import IMessageChannel
    ...
    if channels_config.get("matrix", {}).get("enabled"):
        try:
            from triobot.channels.matrix_channel import MatrixChannel
    ...
    if channels_config.get("sms", {}).get("enabled"):
        try:
            from triobot.channels.sms_channel import SMSChannel
    ...
    if channels_config.get("instagram", {}).get("enabled"):
        try:
            from triobot.channels.instagram_channel import InstagramChannel
    ...
    if channels_config.get("messenger", {}).get("enabled"):
        try:
            from triobot.channels.messenger_channel import MessengerChannel
    ...
    if channels_config.get("line", {}).get("enabled"):
        try:
            from triobot.channels.line_channel import LINEChannel
    ...
    if channels_config.get("reddit", {}).get("enabled"):
        try:
            from triobot.channels.reddit_channel import RedditChannel
    ...
    if channels_config.get("email", {}).get("enabled"):
        try:
            from triobot.channels.email_channel import EmailChannel
    ...
    if config.get("heartbeat", {}).get("enabled"):
        from triobot.channels.heartbeat_channel import HeartbeatChannel
        from triobot.cron.heartbeat import HeartbeatDaemon
